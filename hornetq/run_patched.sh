...
if [ "$0"x != "./run.sh"x ]; then
  echo 'You have to run the script from within the bin directory!!!';
  exit 1;
fi;
...
export JVM_ARGS="$CLUSTER_PROPS -XX:+UseParallelGC -XX:+AggressiveOpts -XX:+UseFastAccessorMethods -Xms512M -Xmx2048M -Dorg.hornetq.logger-delegate-factory-class-name=org.hornetq.integration.logging.Log4jLogDelegateFactory -Dhornetq.config.dir=$CONFIG_DIR -Djava.library.path=."
